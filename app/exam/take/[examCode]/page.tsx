...
export default function ExamTakePage() {
  ...
  const timerProgress = totalDuration > 0 ? ((totalDuration - timeLeft) / totalDuration) : 0;

  // Timer ring SVG values
  const ringRadius = 44;
  const ringCircumference = 2 * Math.PI * ringRadius;
  const ringOffset = ringCircumference * timerProgress;
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#e8edf5] via-[#dfe6f0] to-[#d5dcea] flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="glass-strong shadow-[0_2px_16px_rgba(0,0,0,0.05)] sticky top-0 z-50 flex-shrink-0">
        <div className="h-[56px] flex items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-3">
            <Image src="/logo.png" alt="AbhyasCore" width={90} height={56} className="h-6 w-auto object-contain" />
      ...
      <div className="px-4 md:px-6 pt-4 pb-2 flex-shrink-0">
        <div className="glass-strong rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.05)] p-4 flex items-center gap-5 flex-wrap">
          {/* Circular Timer */}
          <div className="relative w-[72px] h-[72px] flex-shrink-0">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r={ringRadius} fill="none" stroke="#e2e8f0" strokeWidth="5" opacity="0.4" />
              <circle
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <svg className={`w-5 h-5 ${isTimeCritical ? "text-red-500" : isTimeWarning ? "text-orange-500" : "text-indigo-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex-shrink-0">
            <div className={`text-[32px] font-extrabold tabular-nums tracking-wide leading-none ${isTimeCritical ? "text-red-600 anim-pulse-gentle" : isTimeWarning ? "text-orange-600" : "text-slate-900"}`}
              style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              {formatTimer(timeLeft)}
            </div>
            <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 mt-1">
              Minutes:Seconds
              <span className="block text-[9px] text-slate-300 tracking-[0.1em]">TIME REMAINING</span>
            </div>
          </div>
      ...
      <div className="flex-1 flex min-h-0 relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-8">
          ...
          <div className="anim-fade-in flex items-center gap-2.5 mb-4">
          ...
          <div className="glass-strong rounded-[20px] shadow-[0_8px_40px_rgba(0,0,0,0.04)] p-6 md:p-8 mb-5">
            {/* Question Text */}
            <div className="mb-2">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                Multiple Choice Question {currentQ + 1}
              </span>
            </div>
            <p className="text-[16px] md:text-[17px] font-semibold text-slate-900 leading-relaxed mb-8">
              ...
              {currentQuestion?.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex items-center gap-4 rounded-2xl px-5 py-4 cursor-pointer transition-all duration-200 text-left w-full ${
                      isSelected
                  ...
                  >
                    {/* Radio circle */}
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 border-2 ${
                      isSelected
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              disabled={currentQ === 0}
              className="h-10 px-4 rounded-xl flex items-center gap-2 text-[12px] font-bold glass text-slate-600 hover:shadow-md transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            <button
              onClick={clearAnswer}
              className="h-10 px-4 rounded-xl flex items-center gap-2 text-[12px] font-bold glass text-slate-600 hover:shadow-md transition-all"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
