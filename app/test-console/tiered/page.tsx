...
function CustomExamConsoleInner() {
    ...
    authenticatedFetch(`/api/exam/tiered?tier=${tier}&exam=${exam}`)
      ...
      .then(json => {
        if (json.questions) {
          ...
          json.questions.forEach((question: any, i: number) => {
             // using index as key backup if ID is not unique across mixed files
  ...
  const submitExam = async () => {
    ...
    try {
      const totalAllocatedTime = 10800;
      const timeUsed = totalAllocatedTime - timeLeft;

      // Map _safeId answers back to original question IDs for server
      ...
      // Submit to server-side grading endpoint
      ...
      localStorage.setItem(`test_results_${customTestId}`, JSON.stringify({
        ...
        chapterName: result.chapterName || "Custom Generation Drill",
        subject: result.subject || "Mixed Syllabus",
  ...
  if (!data || !searchParams.get("tier")) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-center p-4">
         <AlertCircle className="w-16 h-16 text-red-500 mb-4" />
         <h2 className="text-2xl font-bold text-slate-900 mb-2">Invalid Configuration</h2>
         <p className="text-slate-500 mb-6 max-w-md">No valid chapters were selected or the questions could not be generated.</p>
