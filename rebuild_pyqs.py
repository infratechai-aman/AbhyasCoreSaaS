...
def build_full_paper(year, is_jee):
    prefix  = "jee_main" if is_jee else "neet"
    outfile = os.path.join(RAW_DIR, f"{prefix}_{year}.xml")

    # Distribution: JEE = 30P+30C+30M=90 | NEET = 50P+50C+100B=200
    if is_jee:
        distribution = [
            (physics_files,   30),
            (chemistry_files, 30),
    ...
    actual = len(all_qs)
    print(f"  {outfile}: {actual} questions written")
    return actual

# ── Rebuild ALL 30 files (2010-2024) ────────────────────────────────────────
for year in range(2010, 2025):
    print(f"\n=== Year {year} ===")
    build_full_paper(year, is_jee=True)
    build_full_paper(year, is_jee=False)

print("\nAll PYQ files rebuilt successfully!")
