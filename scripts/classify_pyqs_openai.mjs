...
if (!apiKey) {
  console.error("❌ ERROR: OPENAI_API_KEY is missing from .env.local!");
  console.error("Please add your OpenAI API key to .env.local like this:");
...
async function processFile(filename) {
  ...
  const texts = matches.map(m => m[1].trim().substring(0, 500)); // Limit string length to save tokens
  const subjects = [];
  ...
  // Inject the subjects into the XML content
  let currentIndex = 0;
...
async function run() {
  console.log("🚀 Starting PYQ Classification Cleanup using OpenAI gpt-4o-mini...");
